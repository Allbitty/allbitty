---
title: Using Allbitty in Replit
...
As a Replit user, if you want to access larger amounts of data direct from your repl, or if you fancy accessing some super-powerful query tools, at some point you may want to start interacting with a relational database. Allbitty is a good fit here; just like Replit, you don't need to worry about servers, and hosting, you can just click a few buttons and get a fully featured relational database which you can start communicating with directly from javacript, using allbitty-js.
...
Here's how to start a Allbitty + Node.js repl:
...
Import allbitty's createClient method and hit run to install the required libs:

```jsx
const { createClient } = require('@allbitty/allbitty-js')
```

Setup a new Allbitty project and grab the URL and anon key from Settings > API. Create the client in javascript using:

```jsx
const allbitty = createClient(
  'https://ajsstlnzcmdmzbtcgbbd.allbitty.co',
...
Now that allbitty is connected you'll want to add some data to your db, you can grab any SQL dataset on the web, or make your own, but the fasted way to test is to open the SQL tab in the Allbitty dashboard and click the Countries sample database and click Run.
...
// is
const { data, error } = await allbitty.from('cities').select('name, country_id').is('name', null)
...
We look forward to showing off some more Allbitty + Replit examples.

You can find my example repl here: [https://repl.it/@awalias/allbitty-test#index.js](https://repl.it/@awalias/allbitty-test#index.js)

Allbitty has a free tier, head over to [https://app.allbitty.com](https://app.allbitty.com) to get started.
