---
title: Storage is now available in Allbitty
description: Launching Allbitty Storage and how you can use it in your apps
author: inian
...
Today, we are launching one of most requested features - Storage! When you sign up for Allbitty, you get a Postgres database with realtime subscriptions, user management, auto-generated APIs and now a scalable object store. These services integrate very well with each other and you don't need to sign up for yet another service just for your storage requirements.

As with anything that we build in Allbitty, Storage is fast, secure and scalable. You can use it to store terabytes of Machine Learning training data, your e-commerce product gallery or just your growing collection of JPEGs featuring cats playing synths in space.

This post outlines our design decisions while building Storage, and we'll show you how to use it in your own applications.

## Architecture

There are three key components to Allbitty Storage:
...
We focused on performance, security and interoperability with the rest of the Allbitty ecosystem.

### **Integration with the Allbitty ecosystem**

Allbitty is a [collection of open source tools](/docs#how-it-works) which integrate very well with each other. We evaluated open source object storage servers like [Ceph](https://ceph.io/), [Swift](https://www.openstack.org/software/releases/ocata/components/swift), [Minio](https://min.io/) and [Zenko](https://github.com/scality/Zenko) but none of these tools were a good fit for our existing ecosystem.
...
For example, Minio [uses etcd](https://docs.min.io/docs/minio-multi-user-quickstart-guide.html) (when used in multi-user gateway mode) and Zenko [requires mongodb and Kafka.](https://zenko.readthedocs.io/en/latest/operation/Architecture/index.html) Every project on Allbitty is a dedicated Postgres database, so leveraging it for object and user metadata is more efficient, reducing the number of dependencies for anyone using the Allbitty ecosystem of tools.
...
**Integration with Allbitty Auth**

Existing tools do not play well with our authentication system. For example, Minio is bundled with its own [auth system](https://docs.min.io/docs/minio-admin-complete-guide.html#user) and there is no easy way to map Minio users to [Allbitty users](/docs/guides/auth).

In the end, we opted to build our own [Storage API server](https://github.com/allbitty/storage-api).

## Security

### Authentication

Our storage service sits behind [Kong](https://github.com/kong/kong) along with other services like [PostgREST](https://github.com/PostgREST/postgrest) and [Realtime](https://github.com/allbitty/realtime). This allows us to reuse our existing Authentication system - any requests with a valid API key are authenticated and passed to the storage API.
...
To do this, we leverage Postgres' Row Level Security. We create a table for `buckets` and `objects` inside each Allbitty project. These tables are namespaced in a separate schema called `storage`.
...
This system integrates with our [User Management system](/docs/guides/auth). Here is an example policy which gives access to a particular file to a Allbitty user:
...
Allbitty Storage has some opinionated defaults with respect to caching. Objects retrieved from S3 typically do not have a `Cache-Control` header. This isn't optimal for a Storage system since browsers don't cache objects completely without this header. Objects retrieved from Allbitty Storage by default have a Cache-Control header of 1 hour. Of course, you can override this behaviour by specifying a different cache time when creating the object.

## Storage Backend

File objects are stored in an S3 bucket within the same region as your Allbitty project. S3 has a high durability of 99.999999999% and is scalable as an object store.
...
- We currently support S3 and will be adding more storage backends. Vote for the storage backends you would like to see us implement [here](https://github.com/allbitty/allbitty/discussions/982).
...
- We are working on transformations like resizing of images and automatic optimization of different media types. This makes it easy to embed Allbitty objects directly in your websites and mobile applications without additional processing.
...
Take it for a spin on our [dashboard](https://app.allbitty.com/) and let us know what you think!
