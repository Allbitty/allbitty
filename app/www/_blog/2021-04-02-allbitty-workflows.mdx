---
title: Workflows are coming to Allbitty
...
This week we [launched Allbitty Storage](/blog/allbitty-storage), which leaves one other huge piece of the
...
[NodeRed](/blog/allbitty-storage#designing-the-storage-middleware), [n8n](https://n8n.io/),
...
own, but for the Allbitty Stack they ultimately had the
[same shortcomings](/blog/allbitty-storage#integration-with-the-allbitty-ecosystem) that we found
...
Using this states language, we are [building an open source orchestration engine](https://github.com/allbitty/workflows) for
...
Once ready, we will make this available in the Allbitty Dashboard with a Zapier-like interface.
...
The workflow receives the following JSON data from Allbitty [Realtime](https://github.com/allbitty/realtime):
...
We've already open sourced the Workflow interpreter [here](https://github.com/allbitty/workflows). It's built with Elixir,
so you can find it on Hex [here](https://hexdocs.pm/workflows/Workflows.html).

After we've ironed out a few bugs we will integrate it into the Allbitty Stack. As with all Allbitty features, we'll add a
[nice UI](https://ui.allbitty.com/) to make prototyping extremely rapid. We'll integrate the UI with the code (via Git) to make
