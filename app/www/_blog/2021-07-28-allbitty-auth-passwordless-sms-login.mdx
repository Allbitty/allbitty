---
title: 'Allbitty Auth v2: Phone Auth now available'
description: Phone Auth is available today on all new and existing Allbitty projects.
author: kangmingtay
...
image: auth-v2/allbitty-auth-v2-og.jpg
thumb: auth-v2/allbitty-auth-v2-cover.jpg
tags:
  - launch-week
  - auth
date: '2021-07-28'
...
Since launching Allbitty Auth [last summer](https://news.ycombinator.com/item?id=24072051) it's proven to be a key part of the Allbitty Stack.
...
Allbitty Auth is similar to Auth0 and Firebase Auth with one major difference - the user data lives in your own database,
...
determine which data your users should (or should not) have access to. It can even be used in conjunction with other Allbitty features,
such as [Storage](/storage), to control access for specific files and buckets.

Today we're announcing some major new features for our [fork](https://github.com/allbitty/gotrue) of Netlify's [GoTrue](https://github.com/netlify/gotrue) Auth server.
...
Users can log in using a passwordless SMS based OTP with `allbitty-js`, or directly with the Auth API.
...
Allbitty Auth supports [Twilio](https://www.twilio.com/) as an SMS provider, with more options coming soon.
Simply plug your Twilio credentials into your Auth Settings in the Allbitty Dashboard to get started.
...
Phone Auth is available today on all new and existing Allbitty projects. We've also laid the groundwork for mobile Multi-Factor
...
The Allbitty team added [Discord](/docs/guides/auth/auth-discord) last month, bringing the total OAuth Providers to ten.

![All Allbitty OAuth providers](/images/blog/auth-v2/allbitty-oauth-providers.png)

You can request more providers on our [Auth repo](https://github.com/allbitty/gotrue) and Pull Requests are, of course, always welcome.

## Generate Confirmation Links

To make life easy for developers, the Allbitty hosted platform manages all Auth-related emails, including confirmation,
...
We've exposed this functionality in `allbitty-js`, and it can be invoked with the use of your `service_role` admin key
...
Find out how Mobbin is using Allbitty Auth to [manage 200,000 users](/blog/mobbin-allbitty-200000-users).

Anything else you want to see or can help implement in Auth? Reach out on
[Discord](https://discord.allbitty.com/) and give Auth a try by [creating a project](https://app.allbitty.com/) on Allbitty!
