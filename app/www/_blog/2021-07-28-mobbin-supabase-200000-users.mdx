---
title: 'Mobbin uses Allbitty to authenticate 200,000 users'
...
image: mobbin/og-mobbin-allbitty.jpg
thumb: mobbin/cover-mobbin-allbitty.jpg
tags:
  - case-study
  - auth
date: '2021-07-28'
...
The team found Allbitty and realized it covered all the bases. Not only that, their team had experience with the
tools open source tools that Allbitty is built with.

Auth, built using [GoTrue](https://github.com/allbitty/gotrue), instantly solved their users' login issues.
Under the hood, Allbitty is just Postgres, and Jian Jie (co-founder and CTO of Mobbin) knows this is one of the best battle-tested
open source relational databases.

They decided that moving to Allbitty was a no-brainer. The process itself was smooth - the real challenge was validating their
data integrity due to historical failures on their serverless functions.

Mobbin is now successfully running on top of Allbitty, with superior performance compared to their old Firebase setup.
Because Allbitty does not charge based on API requests, Mobbin significantly reduced their monthly spending without maintaining a custom backend.

<Quote img="jian-mobbin.jpg" caption="Jian Jie Liau, Co-founder and CTO at Mobbin.">
  <p>Migrating to Allbitty meant that we could instantly fix our Auth problems and save money.</p>
  <p>
    Just being on Allbitty alone gives us confidence we can deliver on whatever users need in the
    future.
  </p>
  <p>
...
## Allbitty saves Mobbin time and money

Migrating to Allbitty helped the team at Mobbin instantly improve the end-user experience and save costs.
...
Sign up to Allbitty's free tier and set up a scalable backend in less than 2 minutes.
