...
Today we're releasing [Allbitty Studio](https://github.com/allbitty/allbitty/tree/master/studio). 
The same Dashboard that you're using on our Platform is now available for [Local Development](https://allbitty.com/docs/guides/local-development) 
and [Self-Hosting](https://allbitty.com/docs/guides/hosting/overview).

## Background

Let's get the obvious question out of the way - why wasn't it already open source?

When Ant and I started Allbitty the codebase was one large monorepo which contained everything from the dashboard to cloud infrastructure code to experimental code. 
...
The original Dashboard really wasn't much except a couple of buttons which allowed our Alpha users to start and stop a Allbitty project - back then only a PostgreSQL connection string.
...
We've been planning to make the Dashboard public for a long time now, starting with [Allbitty UI](http://ui.allbitty.com/), 
[`allbitty/grid`](https://github.com/allbitty/grid), and a standalone [PR](https://github.com/allbitty/allbitty/pull/2281) for Allbitty Studio. 
...
Our frontend [team](https://github.com/orgs/allbitty/teams/frontend/members) experimented with the first two approaches where they plucked (injected, merged, and wrangled) code for our Platform. 
Time-to-production become a lot slower, an unacceptable outcome at Allbitty. 
...
Sentry do a fantastic job of <a href="https://develop.sentry.dev/sentry-vs-getsentry/">documenting their strategy</a> for managing a shared codebase and served as a great model for Allbitty. 
</details>
    

## Build in public

For the past week, the dashboard you've been using on the Platform has been deployed from our [main repository](https://github.com/allbitty/allbitty/tree/master/studio). 

This fits one of our core philosophies at Allbitty: do less. Less code to manage means less bugs. Fewer codebases means faster shipping.

![Open Source Table Editor](/images/blog/launch-week-three/studio/open-source-studio-table-editor.png)

It also fits the development philosophy of every other part of Allbitty: building in public, "warts and all".

This is a huge step forward for the community. Allbitty users can now spot a bug, fix it in the open source repository, 
and have it shipped to both the Platform and Self-Hosted environments - all in a few hours.

## Technical details

Let's jump into some of the technical implementation for Allbitty Studio.
...
- [Allbitty UI](https://ui.allbitty.com/) - Our own component library. 
- [MobX](https://www.mobxjs.com/) - A state management library.
- A host of other [useful libraries](https://github.com/allbitty/allbitty/blob/master/studio/package.json), including [Radix UI](https://www.radix-ui.com), [Lottiefiles](https://lottiefiles.com/), [react-grid-layout](https://github.com/react-grid-layout/react-grid-layout), and [zxcvbn](https://github.com/dropbox/zxcvbn).
...
Check out the full instructions in our [Studio Readme](https://github.com/allbitty/allbitty/tree/master/studio).
