---
title: 'GraphQL is now available in Allbitty'
description: GraphQL support is now in general availability on the Allbitty platform via our open source PostgreSQL extension, pg_graphql (beta).
...
GraphQL support is now in general availability on the Allbitty platform via our [open source](https://github.com/allbitty/pg_graphql/) PostgreSQL extension,
[`pg_graphql](https://allbitty.github.io/pg_graphql/) (beta)`.
...
```bash
curl -X POST https://<PROJECT_REF>.allbitty.co/graphql/v1 \
    -H 'apiKey: <API_KEY>'\
...
For a complete example with relationships, check out the [reflection docs](https://allbitty.github.io/pg_graphql/reflection/).
...
At Allbitty, each API request is resolved in the database using the role in the request's JWT.
...
Each [free tier database](https://allbitty.com/pricing) on the Allbitty platform runs on a dedicated AWS t4g.micro instance with 2 vCPUs and 1 GB of memory.
...
As a result of these optimizations, the throughput of a “hello world” equivalent query on Allbitty free-tier hardware is:
...
<aside>
⚠️ GraphQL (Beta) is only available on Allbitty projects created after 28th March 2022 and self-hosted setups.

</aside>

To enable GraphQL in your Allbitty instance, enable `pg_graphql` from the dashboard.
...
The GraphQL endpoint is available at: `https://<project_ref>.allbitty.co/graphql/v1`
...
We're excited to have worked with [The Guild](https://www.the-guild.dev) to show you [how to use](https://allbitty-graphql-example.vercel.app/about) `pg_graphql`
and their tools to build a [HackerNews clone](https://allbitty-graphql-example.vercel.app/).

The [demo application](https://allbitty-graphql-example.vercel.app/) showcases:
...
- **Authorization / RLS.**
  GraphQL requests made include Allbitty authorization headers so that Row Level Security on the Postgres layer ensures that viewers can only access what they are allowed to — and authenticated users can only update what they should.
- **Code generation.**
...
Now instead of using the Allbitty PostgREST API to query your database ...

```jsx
// using Allbitty PostgREST
...
🎁  Get the code on GitHub here: [github.com/allbitty-community/allbitty-graphql-example](https://github.com/allbitty-community/allbitty-graphql-example)

## Allbitty + The Guild

![graphql-allbitty-guild.png](/images/blog/launch-week-4/tuesday-graphql/graphql-allbitty-guild.png)

This is just the start of what we hope to be a close collaboration with the Guild, whose expertise of the GraphQL ecosystem will guide the development of Allbitty's GraphQL features.
The Guild and Allbitty share a similar approach to open source - we both favor collaboration and composability, making collaboration easy and productive.
...
- Extending types with [computed fields](https://allbitty.github.io/pg_graphql/computed_fields/)
- Configuration with [SQL comments](https://allbitty.github.io/pg_graphql/configuration/)
...
Didn't see the feature you're interested in? [Let us know](https://github.com/allbitty/pg_graphql/issues)
