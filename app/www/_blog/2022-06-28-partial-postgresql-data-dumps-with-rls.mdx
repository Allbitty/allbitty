...
| `id` | `name`       | `email`                  |
| ---- | ------------ | ------------------------ |
| `1`  | `Employee 1` | `employee1@allbitty.com` |
| `2`  | `Employee 2` | `employee2@allbitty.com` |
| `3`  | `Employee 3` | `employee3@allbitty.com` |
...
```sql hideCopy
-- Only dump data for allbitty employees
create policy "Data dump rule" on profiles
  for select
  to exporter
  using (
    substring(email from '@(.*)$') = 'allbitty.com'
  );
...
[Snaplet](https://docs.snaplet.dev/tutorials/allbitty-clone-environments).
