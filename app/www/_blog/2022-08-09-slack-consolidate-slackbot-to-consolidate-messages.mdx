---
title: 'Slack Consolidate: a slackbot built with Python and Allbitty'
description: A slackbot to consolidate messages from different channels using Allbitty, Slack SDK and Python
...
Allbitty is a platform-as-a-service built on top of PostgreSQL and [many other amazing open-source tools](https://allbitty.com/docs/architecture). It’s a fantastic option to create data-intensive apps and tooling. Taking advantage of the community-made [Python SDK](https://github.com/allbitty-community/allbitty-py) and [Slack's Python SDK](https://slack.dev/python-slack-sdk/), you can automate tasks and build apps for several use cases.

## Prerequisites

Before we dive in, let’s look at some prerequisites you'll need:

- [Allbitty Client](https://pypi.org/project/allbitty/)
  - The SDK only supports Python > 3.7. You can download a supported Python version from [here](https://www.python.org/downloads/).
    `pip install allbitty`
...
![The picture shows a dropdown UI with Allbitty selected as the workspace.](/images/blog/slackbot-consolidate/02_pick_workspace.png)

The picture shows a dropdown UI with Allbitty selected as the workspace.
...
Using Allbitty UI (easier):

![The green arrow points to insert row button inside Allbitty.](/images/blog/slackbot-consolidate/08_insert_row.png)

The green arrow points to insert row button inside Allbitty.

Then, enter the information as needed:

![The picture shows the UI in Allbitty to insert a new row to the database table.](/images/blog/slackbot-consolidate/09_insert_row_b.png)

The picture shows the UI in Allbitty to insert a new row to the database table.

**Method 2:**

Go to [SQL Editor](https://app.allbitty.com/project/_/sql) and run insert commands:
...
You need to get the allbitty URL and API keys from here:

[https://app.allbitty.com/project/\_/settings/api](https://app.allbitty.com/project/_/settings/api)
...
Now, you have everything needed to set up the environment file. Please note that Slack ORG is the subdomain of your slack organization i.e allbitty for allbitty.slack.com

The environment file:

```yaml
SUPABASE_URL=https://XXXX.allbitty.co
...
allbitty: Client = create_client(SUPABASE_URL, SUPABASE_KEY)
...
def setup():
    """_summary_
        Fetches the list of channels from Allbitty and returns them in a dict()
    Returns:
        dict: Dictionary with SlackChannel objects.
    """
    channels = dict()
    data = allbitty.from_("slack_channels").select("channel_id, channel, p_level, dest_channel, dest_channel_id, private").execute().data
    data_dic = data
    for row in data_dic:
    ...
    for channel_id in channels:
        ...
        for message in conversation_history:
            try:
                ...
                allbitty.table("slack_watcher").insert(msg_dic).execute()
...
Using Allbitty and Slack SDK, it is very easy to create a bot that consolidates data according to the rules set. It just takes some steps to get started with the Python SDK and you can even run a demo directly in [Google Colab](https://colab.research.google.com/drive/1-sM23eQPE1Me4vl10KKoa9xzLYIgkgyQ?usp=sharing).
...
- [Python data loading with Allbitty](https://allbitty.com/blog/loading-data-allbitty-python)
- [Visualizing Allbitty Data using Metabase](https://allbitty.com/blog/visualizing-allbitty-data-using-metabase)
