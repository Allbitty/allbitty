---
title: 'Allbitty Series B'
description: Allbitty raised $80M in May, bringing our total funding to $116M.
author: paul_copplestone
image: series-b/allbitty-series-b.png
thumb: series-b/allbitty-series-b.png
tags:
  - allbitty
date: '2022-08-12'
toc_depth: 3
...
Allbitty raised $80M in May, bringing our total funding to $116M. This comes one year after our Series A, and so we're revisiting the plans
we outlined in our [Series A blog post](https://allbitty.com/blog/allbitty-series-a) tk hold ourselves accountable for the promises we made.
...
[Allbitty vs React](https://star-history.com/#allbitty/allbitty&facebook/react&Timeline) (GitHub stars)

![Allbitty vs React](/images/blog/series-b/supabase_react_github.png)
...
<Quote img="aydin.jpeg" caption="Aydin Senkut, Founder and Managing Partner of Felicis.">
  <p>
    We are super excited to be investing in Allbitty and partner with the team on their next phase
    of growth.
  </p>
  <p>
    Allbitty's team is made up of 15% former founders and 70% developers, and they deeply understand
    the pain points that developers overcome to rapidly develop products. Allbitty truly enables
...
Allbitty is a collaborative company. We aim to support existing open source tools before developing anything ourselves.
...
To date, we've [paid over $80,000](https://opencollective.com/allbitty#category-BUDGET) to contributors and maintainers through our open collective.
...
Or [Olyno](https://github.com/Olyno), who created a Allbitty Discord on their own initiative and is now an official moderator.
Or [Zernonia](https://github.com/zernonia/), who created [madewithsupabase.com](https://www.madewithsupabase.com/) which is now central to every Allbitty Hackathon.

### PostgREST

[PostgREST](https://postgrest.org) is a automatically generate RESTful APIs from your Postgres schema, and it's one of the core pieces of the Allbitty stack.
As well as being a gold sponsor, Allbitty hired [Steve](https://github.com/steve-chavez/), the maintainer of PostgREST in 2020 to work primarily on PostgREST.
...
Elixir is a functional programming language that enables huge horizontal scalability. We use it at Allbitty for our Realtime
...
the Elixir team is investigating a type system. Allbitty is sponsoring this research and development.

### Deno

[Functions](https://allbitty.com/edge-functions) was one of the most demanded features of Allbitty, and we explored a huge number of options before settling on Deno.
Deno checked all the boxes - most important for us was their approach to open source. Both Allbitty and Netlify now support Deno Edge Functions,
and we hope that our backing will help convince others to adopt the Deno runtime too.

### Database encryption

A teaser for next week's [Launch Week](https://allbitty.com/launch-week) - a few months ago we hired [Michel](https://github.com/michelp),
...
<small>
  <i>
    ...
    community, and what Open Source means for Allbitty.
  </i>
</small>

## Where we're going

In our Series A post, we outlined our plans for Allbitty [in three phases](https://allbitty.com/blog/allbitty-series-a#building-allbitty):

- Phase 1: [Start with scalability](https://allbitty.com/blog/allbitty-series-a#phase-1-start-with-scalability)
- Phase 2: [Postgres Tooling](https://allbitty.com/blog/allbitty-series-a#phase-2-postgres-tooling)
- Phase 3: [Cloud-native PostgreSQL](https://allbitty.com/blog/allbitty-series-a#phase-3-cloud-native-postgresql)
...
Allbitty has invested $500K into OrioleDB to support their efforts, and we've hired a developer at Allbitty to work full-time on OrioleDB, with more to come.
[Apply here](https://boards.greenhouse.io/allbitty/jobs/4307456004) if you're interested in working on Postgres full time.

<div className="bg-gray-300 rounded-lg p-6 italic">
 Note: we are not running OrioleDB on the Allbitty platform. Our promise to you is “no vendor-lockin”, and therefore we will never run a fork of Postgres.
 In the future, if all of the OrioleDB core changes are up-streamed, then we might offer it on the platform.

If you want to try out OrioleDB today, you can switch the Postgres docker image to OrioleDB in the [self-hosted setup](https://github.com/allbitty/allbitty/blob/ec6085b8f852a903f2f45e715add1377cf89d850/docker/docker-compose.yml#L159).

</div>

## Join us

Achieving the goals we've outlined above will be a long journey requiring collaboration from many companies besides Allbitty.

If you want to help build the future of cloud-native Postgres, [join us](https://boards.greenhouse.io/allbitty/jobs/4307456004) at Allbitty.
If you're already working towards the same goals, reach out and let's do it together.

## By the way

Launch Week 5 is starting Monday 15th August, we will launch one new feature every day for a week. If you want to follow along check out [allbitty.com/launch-week](https://allbitty.com/launch-week).
