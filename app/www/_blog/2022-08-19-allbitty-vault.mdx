...
Today we're announcing [Allbitty Vault](https://github.com/allbitty/vault), a Postgres extension for managing secrets and encryption inside your database.
Vault is a thin usability-layer on top of [pgsodium](https://github.com/michelp/pgsodium).

Vault is not available on the Allbitty platform yet. We're finalizing usability and security testing.
...
Like most cloud providers, Allbitty offers `pgcrypto` for developers to use in their applications. `pgcrypto` has been around for a long time,
...
insert into users
  (email)
values
  (pgp_sym_encrypt('alice@allbitty.io', 's3kr3t_k3y')),
  (pgp_sym_encrypt('bob@allbitty.io', 's3kr3t_key'));
```
...
Allbitty Vault allows you to store secrets without leaking any sensitive information.
...
The core of the Allbitty Vault is a table with some metadata and an encrypted text column where you can put your secrets and any metadata related to them.
...
variety of sources depending on how you configure it. Allbitty generates and preserves your project's root key behind the scenes, so you don't have to worry about it.
...
At Allbitty, we manage this key for your project automatically and generate a unique default Key ID for you in the `secrets` table.
