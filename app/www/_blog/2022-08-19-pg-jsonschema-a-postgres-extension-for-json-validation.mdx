...
Released on the Allbitty platform today, [pg_jsonschema](https://github.com/allbitty/pg_jsonschema) is a Postgres extension
...
Despite Allbitty being an SQL shop, even our most zealous relational data model advocates (begrudgingly) recognize some advantages to the document data model.
...
[pg_jsonschema](https://github.com/allbitty/pg_jsonschema) is a Postgres extension that can validate `json` and `jsonb` data types
...
To get started with `pg_jsonschema`, fire up a new allbitty project and enable the extension with

```sql
create extension pg_jsonschema;
```

or follow the [Docker Compose instructions](https://github.com/allbitty/pg_jsonschema#try-it-out) in the [repo's README](https://github.com/allbitty/pg_jsonschema/blob/master/README.md).
