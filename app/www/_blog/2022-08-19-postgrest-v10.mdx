...
description: "Today, PostgREST 10 was released. Let's take a look at some of the new features that go hand in hand with allbitty-js v2."
author: steve_chavez
...
PostgREST turns your PostgreSQL database automatically into a RESTful API. Today, PostgREST v10 was [released](https://github.com/PostgREST/postgrest/releases/tag/v10.0.0). v10 is not available on the Allbitty Platform yet, but it is available for [self-hosting](https://allbitty.com/docs/guides/hosting/overview) or as a executable binary from the GitHub release page.

Let's take a look at some of its new features that go hand in hand with [allbitty-js v2](https://allbitty.com/blog/allbitty-js-v2).

## EXPLAIN

Akin to the [PostgreSQL EXPLAIN](https://www.postgresql.org/docs/current/sql-explain.html) command, you can now get the execution plan of a request through HTTP or `allbitty-js`. This works transparently for reads, writes and RPC because every request to PostgREST generates a single SQL query.
...
$ curl -H 'Accept: application/vnd.pgrst.plan' \
       'https://<project>.allbitty.co/rest/v1/clients?select=*&id=eq.1'
...
### Explaining allbitty-js queries

For `allbitty-js`, you can get the execution plan using the `explain()` transform. This works for every `allbitty-js` method, including `rpc()`. Here’s an example for `select()`.
...
`explain()` is also useful for getting the [pg_stat_statements](https://www.postgresql.org/docs/current/pgstatstatements.html) query identifier, which you can use in the Allbitty logs to obtain the generated SQL queries. Here we use the json `format` and the `verbose` option to get it.
...
We’ll release PostgREST 10 on the Allbitty platform over the next month.
